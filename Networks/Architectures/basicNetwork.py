...
class UNetLight(nn.Module):
    ...
    def __init__(self, param):
        ...
        self.final_conv = nn.Sequential(nn.Conv2d(2*self.n_channels, 1, kernel_size=1),
                                        nn.Dropout(p=0.25))
...
class UNet(nn.Module):
    def __init__(self, param):
        ...
        self.decoder3 = self.decoder_block(2*self.n_channels * 8  , self.n_channels * 4)
        self.decoder2 = self.decoder_block(2*self.n_channels * 4 , self.n_channels * 2)
        self.decoder1 = self.decoder_block(2*self.n_channels * 2 , self.n_channels)
